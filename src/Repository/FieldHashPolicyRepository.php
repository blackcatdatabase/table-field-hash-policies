...
use BlackCat\Database\Packages\FieldHashPolicies\Repository\FieldHashPolicyRepositoryInterface;
...
class FieldHashPolicyRepository implements FieldHashPolicyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\FieldHashPolicies\Definitions::class);
    ...
    public function getByEntityTableAndFieldNameAndEffectiveFrom(string $entityTable, string $fieldName, \DateTimeInterface|string $effectiveFrom, bool $asDto = false): array|\BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto|null {
        return $this->getByUnique([ 'entity_table' => $entityTable, 'field_name' => $fieldName, 'effective_from' => $effectiveFrom ], $asDto);
    }
    ...
    public function getIdByEntityTableAndFieldNameAndEffectiveFrom(string $entityTable, string $fieldName, \DateTimeInterface|string $effectiveFrom) {
        $row = $this->getByEntityTableAndFieldNameAndEffectiveFrom($entityTable, $fieldName, $effectiveFrom, false);
