...
namespace BlackCat\Database\Packages\FieldHashPolicies\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\FieldHashPolicies\Definitions;
use BlackCat\Database\Packages\FieldHashPolicies\Criteria;
use BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto as Dto;
use BlackCat\Database\Packages\FieldHashPolicies\Mapper\FieldHashPolicyDtoMapper as RowMapper;
...
final class FieldHashPolicyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\FieldHashPolicies\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\FieldHashPolicies\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto|null */
    public function getByEntityTableAndFieldNameAndEffectiveFrom(string $entityTable, string $fieldName, \DateTimeInterface|string $effectiveFrom, bool $asDto = false): array|\BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'field_name' => $fieldName, 'effective_from' => $effectiveFrom ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\FieldHashPolicies\Mapper\FieldHashPolicyDtoMapper::fromRow($row);
    }
    public function existsByEntityTableAndFieldNameAndEffectiveFrom(string $entityTable, string $fieldName, \DateTimeInterface|string $effectiveFrom): bool {
        $where = 't.' . Ident::q($this->db, 'entity_table') . ' = :uniq_entity_table' . ' AND ' . 't.' . Ident::q($this->db, 'field_name') . ' = :uniq_field_name' . ' AND ' . 't.' . Ident::q($this->db, 'effective_from') . ' = :uniq_effective_from';
        return $this->exists($where, [ 'uniq_entity_table' => $entityTable, 'uniq_field_name' => $fieldName, 'uniq_effective_from' => $effectiveFrom ]);
    }
    /** @return int|string|null */
    public function getIdByEntityTableAndFieldNameAndEffectiveFrom(string $entityTable, string $fieldName, \DateTimeInterface|string $effectiveFrom) {
        $row = $this->getByEntityTableAndFieldNameAndEffectiveFrom($entityTable, $fieldName, $effectiveFrom);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto|null {
