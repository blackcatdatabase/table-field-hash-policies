...
namespace BlackCat\Database\Packages\FieldHashPolicies\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\FieldHashPolicies\Dto\FieldHashPolicyDto;
use BlackCat\Database\Packages\FieldHashPolicies\Mapper\FieldHashPolicyDtoMapper;
 ...
 */
final class FieldHashPoliciesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private FieldHashPolicyRepository $fieldHashPolicyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'field_hash_policies' => $this->fieldHashPolicyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\FieldHashPolicies\ModuleException("Record {$idStr} not found for locking.");
            }
