...
namespace BlackCat\Database\Packages\FieldHashPolicies\Joins;
 ...
 */
final class FieldHashPoliciesJoins {
    ...
    /**
     * FK: field_hash_policies -> hash_profiles
     * LEFT JOIN vw_hash_profiles AS $as ON $as.id = $alias.profile_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinHashProfiles(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_hash_profiles AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.profile_id' . ' ', []];
    }
