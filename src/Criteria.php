...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'entity_table', 'field_name', 'profile_id', 'effective_from', 'notes' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'entity_table', 'field_name', 'notes' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        $x = [ 'id', 'entity_table', 'field_name', 'profile_id', 'effective_from', 'notes' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\FieldHashPolicies\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\FieldHashPolicies\Definitions::softDeleteColumn();
