...
> Lineage: 1 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII flags: 1 · Changelog: fresh (18 d)
...
| Pkg folder | [packages\field-hash-policies](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\field-hash-policies) | Repo location |
...
graph LR
  ...
  field_hash_policies["field_hash_policies"]:::center
  field_hash_policies -->|FK| hash_profiles["hash_profiles"]:::link
  linkStyle 0 stroke:#ff6b6b,stroke-width:3px,opacity:0.92;
```

- Outbound (depends on): "hash_profiles"
...
| Engine | Support |
| --- | --- |
| mysql | ✅ schema(4)<br/>✅ views(1)<br/>⚠️ seeds |
...
- `effective_from` – default=CURRENT_TIMESTAMP(6)
...
pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package field-hash-policies -Apply
